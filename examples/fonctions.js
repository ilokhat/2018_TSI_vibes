			...
			function createMaterial(width, value){
				...
				var texture = itowns.THREE.ImageUtils.loadTexture( "strokes/small.png" );
				//var texture2 = THREE.ImageUtils.loadTexture( "strokes/paint-brush.png" );
				var texture2 = itowns.THREE.ImageUtils.loadTexture( "strokes/"+value+".png" );
				var texture3 = itowns.THREE.ImageUtils.loadTexture( "strokes/"+value+".png" );
				//var texture3 = THREE.ImageUtils.loadTexture( "strokes/chalk1-155-142.png" );
				var paper = itowns.THREE.ImageUtils.loadTexture("paper2.png");
				/*texture.wrapS = THREE.RepeatWrapping;
				texture.repeat.set( 0.1, 0.1 );
				texture.anisotropy = 10;
				texture.anisotropy = 10;*/
				var color = new itowns.THREE.Color().setHex(params.color_aretes_focus.replace("#", "0x"));

				//Materiel appliqué à toutes les géométries de la couche
				var material = new itowns.THREE.ShaderMaterial( {

					uniforms: {
						time: { value: 1.0 },
						thickness : { value: width },
						resolution: { value: new itowns.THREE.Vector2(window.innerWidth,window.innerHeight) }, // todo: vraie resolution
		   ...
		   //crée les six points pour deux triangles (un quad) à partir d'une arête
			function createQuad(pt1,pt2){
				...
				geometry.addAttribute( 'position', new itowns.THREE.BufferAttribute( vertices, 3 ) );
				geometry.addAttribute( 'position2', new itowns.THREE.BufferAttribute( vertices2, 3 ) );
			...
			function epaisseurAretes(value, arrayQuads)	{
				for (i=0;i<arrayQuads.length;i++){
					arrayQuads[i].material.linewidth = value;
